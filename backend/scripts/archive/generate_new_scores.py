...
profiles = profiles[~profiles['society_name'].str.contains('Test Account', case=False)]

# Fix missing filled_at with placed_at or a default time so pandas doesn't ignore the trades
...
# The user explicitly states ADMN is "purely corrective". We will just strip it out to avoid skewing tracking.
...
for _, profile in profiles.iterrows():
    pid = profile['id']
    # If the corrective trade lacked both placed_at and filled_at, it's NaT. 
    # Fill any remaining NaT with the competition start so they instantly get applied.
    ...
    for ts in prices.index:
        ...
        if day_key not in daily_equity_map:
            ...
            if ts_time < existing['firstTime']:
                existing['first'] = eq
                existing['firstTime'] = ts_time
            if ts_time > existing['lastTime']:
                existing['last'] = eq
                existing['lastTime'] = ts_time
    ...
    minSignificantSamples = 120
    highConfidenceSamples = 504
    
    confidenceFactor = 0.0
    if snapshotCount < minSignificantSamples:
        confidenceFactor = min(snapshotCount / minSignificantSamples, 1.0) * 0.3
    elif snapshotCount < highConfidenceSamples:
        confidenceFactor = 0.3 + ((snapshotCount - minSignificantSamples) / (highConfidenceSamples - minSignificantSamples)) * 0.5
    else:
        confidenceFactor = 0.8 + min((snapshotCount - highConfidenceSamples) / highConfidenceSamples, 0.5) * 0.2
    ...
    if max_dd < 1.0 and confidenceFactor < 0.3:
        drawdownScore = rawDrawdownScore * 0.85
        
    rawVolatilityScore = max(0, 20 * math.exp(-volatility / 20))
    volatilityScore = rawVolatilityScore
    if volatility < 5 and confidenceFactor < 0.3:
        volatilityScore = rawVolatilityScore * 0.7
    ...
    for i, day in enumerate(sortedDays):
        day_data = daily_equity_map[day]
        dailyEquities.append(day_data['last'])
        if i == 0:
            ret = ((day_data['last'] - day_data['first']) / day_data['first']) * 100 if day_data['first'] > 0 else 0
        else:
    ...
    if len(dailyEquities) >= 3:
        n_eq = len(dailyEquities)
        xMean = (n_eq - 1) / 2
        yMean = sum(dailyEquities) / n_eq
        ssTotal = 0
        ssResidual = 0
        sxy = 0
        sxx = 0
        for i in range(n_eq):
            ...
            sxx += (i - xMean) * (i - xMean)
            ssTotal += (dailyEquities[i] - yMean) ** 2
        slope = sxy / sxx if sxx > 0 else 0
        intercept = yMean - slope * xMean
        for i in range(n_eq):
            predicted = intercept + slope * i
            ssResidual += (dailyEquities[i] - predicted) ** 2
    ...
    buySellScore = 0
    if total_prof_trades > 0 and sellTrades > 0:
        sellRatio = sellTrades / total_prof_trades
        if 0.15 <= sellRatio <= 0.7:
            buySellScore = 15
        elif sellRatio < 0.15:
            buySellScore = (sellRatio / 0.15) * 10
        else:
            buySellScore = max(0, 15 - ((sellRatio - 0.7) / 0.3) * 15)
    elif total_prof_trades > 0 and sellTrades == 0:
        buySellScore = 3
    ...
    results.append({
        ...
        'Old_Score': profile['competition_score'],
        'New_Score': round(final_score),
        'True_Equity': round(true_equity, 2),
        'Diff': round(final_score - profile['competition_score'])
    })

df = pd.DataFrame(results)
print("\n=== FINAL RECALCULATED SCORES ===")
print(df.sort_values('New_Score', ascending=False).to_string(index=False))
